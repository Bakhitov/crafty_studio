...
- Ark/Seedream: для Seedream 3.x в I2I учитывается только одно входное изображение; для Seedream 4.0 — поддерживаются несколько (массива data URL) (`app/actions/image/edit.ts`).
- UI: при выборе Seedream 3.x и наличии >1 входной картинки показываем ошибку и блокируем генерацию (`components/nodes/image/transform.tsx`).
 - Обновлён биллинг: Seedream 3.0 T2I — $0.03/шт (как SeedEdit 3.0 I2I) (`lib/models/image/index.ts`).

#### 🎬 Видео

- Добавлены модели Ark Seedance: `seedance-1-0-lite-t2v-250428`, `seedance-1-0-lite-i2v-250428`, `seedance-1-0-pro-250528` с адаптером и поллингом задач (`lib/models/video/ark.ts`, `lib/models/video/index.ts`).
- Биллинг Seedance: Lite T2V/I2V — $0.0009 за 1К токенов; Pro I2V/T2V — $0.00125 за 1К токенов (`lib/models/video/index.ts`).
- Seedance async flow: добавлены эндпоинты создания и статуса задач (`app/api/v1/video/tasks/*`), server action финализации (`app/actions/video/finalize.ts`) и ветка в `VideoTransform` только для Seedance.
...
- For #61 ([@haydenbleasel](https://github